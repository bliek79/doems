...
def build_planner_preview(
    ...
    # Production policy v2: explicit external-control discharge keeps the
    # configured base reserve, but forecast demand is not an enforceable hold
    # floor while the device remains in self_consumption.
    required_min_soc = min_soc_limit + (
        reserve_kwh / battery_capacity * 100.0
    )
    ...
    # Baseline physical self_consumption projection.
    minimum_stored_kwh = battery_capacity * min_soc_limit / 100.0
    max_stored_kwh = battery_capacity * max_soc_limit / 100.0
    start_soc = (
        max(min_soc_limit, min(max_soc_limit, float(soc)))
        if soc is not None
        else min_soc_limit
    )
    baseline_stored_kwh = battery_capacity * start_soc / 100.0
    ...
    for row in price_rows:
        ...
        solar_charge_input = min(
            solar_surplus,
            charge_input_limit,
            max(0.0, max_stored_kwh - baseline_stored_kwh) / charge_eff,
        )
        ...
        support_stored_headroom = min(
            ...
            max(0.0, max_stored_kwh - baseline_stored_kwh),
        )
        ...
        baseline_min_soc = min(
            baseline_min_soc,
            baseline_stored_kwh / battery_capacity * 100.0,
        )

    baseline_grid_import_kwh = sum(
        item["grid_home_kwh"] for item in baseline_grid_rows
    )
    first_min_soc_time = baseline_grid_rows[0]["time"] if baseline_grid_rows else None

    # Shift only unavoidable future grid import to an earlier window when
    # battery round-trip delivery is actually cheaper than direct future import.
    ...
    # Ordinary demand coverage is no longer mislabeled as hard safety charge.
    ...
    return {
        ...
        "planner_preview_note": (
            "Production policy v2 projecteert fysieke self_consumption: "
            "woningontlading mag tot minimum-SOC doorlopen. Alleen latere "
            "onvermijdelijke netimport wordt eerder bijgeladen wanneer dat na "
            "verliezen goedkoper is; reserve blijft context voor expliciete control-acties."
        ),
    }
