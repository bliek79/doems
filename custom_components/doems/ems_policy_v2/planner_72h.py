...
def build_72h_plan_preview(
    ...
    def _dynamic_reserve(index: int) -> tuple[float, float, datetime | None]:
        ...
        # Production policy v2: demand until usable solar remains planning
        # context. The enforceable reserve for explicit external-control actions
        # is the fixed hardware + software reserve; normal self_consumption
        # cannot be held at a higher calculated SOC without a real control action.
    ...
    for index, row in enumerate(rows):
        ...
        # 3) Economic self-consumption support. This explicit temporary charge
        # action exists only when it shifts otherwise unavoidable later direct
        # grid import to a cheaper earlier charge window.
        ...
        # to the configured technical minimum regardless of planning reserve or
        # trade intent. Any reserved trade energy consumed by the home is no
        # longer available for later export.
        ...
        self_consumption_headroom_soc = soc_end - min_soc_limit
        # Natural self_consumption safety is distance to the device minimum.
        # The higher control reserve applies only to explicit external discharge.
        ...
        plan.append(
            {
                ...
                "self_consumption_floor_soc": round(min_soc_limit, 1),
    ...
    return {
        ...
        "auto_plan_72h_note": (
            "Production policy v2 scheidt planningbehoefte van afdwingbare "
            "control-reserve. Normale self_consumption ontlaadt fysiek tot het "
            "technische minimum. Goedkope bijlading wordt alleen als expliciete "
            "tijdelijke control-actie gepland wanneer latere netimport duurder is."
        ),
    }
