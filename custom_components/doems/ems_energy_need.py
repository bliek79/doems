...
def build_energy_need_analysis(
    ...
    """Build the frozen EMS energy balance on native 15-minute forecast slots.

    Usable solar is deliberately conservative in this first observational
    version: the first slot of eight consecutive quarter slots (two hours) where solar
    production is at least equal to the forecast home consumption.
    """
    now_utc = (now or dt_util.utcnow()).astimezone(dt_util.UTC)
    reserve_percent = max(0.0, min(30.0, float(settings.software_reserve_percent)))
    ...
    # Alpha76 requires two consecutive usable solar hours. Native 15-minute
    # DOEMS keeps that same two-hour requirement as eight consecutive slots.
    for index in range(len(rows) - 7):
        window = rows[index : index + 8]
        if window[0]["time"] < current_quarter:
            continue
        if all(
            item["home"] is not None
            and item["solar"] is not None
            and item["solar"] >= item["home"]
            and item["solar"] > 0
            for item in window
        ):
            usable_index = index
    ...
    for index, row in enumerate(rows):
        ...
        fraction = 1.0
        if row["time"] == current_quarter:
            elapsed_seconds = (now_utc - current_quarter).total_seconds()
            fraction = max(0.0, min(1.0, 1.0 - elapsed_seconds / 900.0))
    ...
    if soc is not None:
        ...
        available_battery_kwh = settings.battery_capacity_kwh * usable_soc / 100.0
    ...
    return {
        ...
        "energy_need_usable_solar_rule": (
            "eerste van acht opeenvolgende kwartieren (twee uur) waarin solar >= woningverbruik"
        ),
