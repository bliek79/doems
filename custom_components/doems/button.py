...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    async_add_entities([DOEMSCaptureAlpha41SolarFreezeButton(entry, _manager(hass, entry))])


class DOEMSCaptureAlpha41SolarFreezeButton(ButtonEntity):
    """Capture the immutable Alpha41 Solar reference snapshot once."""

    _attr_has_entity_name = False
    _attr_name = "DOEMS Capture Alpha41 Solar Freeze"
    _attr_unique_id = "doems_capture_alpha41_solar_freeze"
    _attr_suggested_object_id = "doems_capture_alpha41_solar_freeze"
    ...
    def available(self) -> bool:
        return self.manager.capture_available
    ...
    async def async_press(self) -> None:
        await self.manager.async_capture()
