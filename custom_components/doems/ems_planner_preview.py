...
def build_planner_preview(
    ...
    charge_eff = max(0.50, min(1.00, float(settings.charge_efficiency_percent) / 100.0))
    discharge_eff = max(0.50, min(1.00, float(settings.discharge_efficiency_percent) / 100.0))
    roundtrip_eff = charge_eff * discharge_eff
    min_margin = max(0.0, float(settings.minimum_trade_margin_eur_per_kwh))
    ...
    required_min_soc = float(settings.technical_min_soc_percent) + (
    ...
    for row in safety_candidates:
        ...
        capacity_kwh = settings.max_charge_power_w / 1000.0 * fraction * charge_eff
        if capacity_kwh <= 0:
            continue
    ...
    if soc is not None:
        free_capacity_kwh = (
            ...
            * max(0.0, float(settings.max_soc_percent) - float(soc))
            / 100.0
        )
    ...
    return {
        ...
        "planner_preview_assumed_max_charge_power_w": int(settings.max_charge_power_w),
